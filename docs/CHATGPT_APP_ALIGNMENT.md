...
| Privacy policy (categories, purposes, recipients, retention, user controls) | ❌ | Not yet published (gap 3). |
...
| Data from App Requests used only to serve the request / legal compliance | ✅/⚠️ | Tool calls are executed and results returned; query history is retained server-side for the refactor agent. That retention must be disclosed in the privacy policy (gap 3). |
...
2. **Per-tool authorization on `/mcp`** — today any valid token can call any
   tool. Map JWT scopes → tool allowlist (e.g. `sql.query` → read-only tools;
   `pipeline.admin` → dbt/pipeline/schedule; `dashboard.admin` → Metabase),
   reusing `opensnow-auth` scopes already enforced on the REST routes.
3. **Privacy policy** — publish at a stable URL (e.g.
   `opensnow.ebdsweden.com/privacy`): what we store (warehouse data the user
   loads, query history incl. SQL text, dbt model sources, dashboard
   definitions), why (serving requests; query-history powers the refactor
   agent), recipients (Metabase when dashboards are published), retention,
   and user controls (`reset-runtime-state` clears query history).
...
5. **Data-minimization pass on outputs** — strip user identifiers from
   `query_history` responses by default; keep result payloads free of trace
   IDs / internal timestamps that aren't relevant to the user's request.
...
## What changed in this pass (2026-06-12)
