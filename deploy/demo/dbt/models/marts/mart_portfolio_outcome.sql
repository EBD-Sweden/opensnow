...
--
-- Proxies (documented, approximate): equity & funds -> real GDP growth (the
-- currency-neutral economy benchmark); insurance & pensions -> 50% bonds + 50%
-- economy (balanced); cash -> real money-market return; bonds -> real bond yield.
with mix as (
  select geo, year,
...
joined as (
  select m.geo, m.year,
    ...
    round(
      ...
      + m.w_equity  * r.gdp_real_growth_pct
      + m.w_pension * (0.5 * r.real_bond_return_pct + 0.5 * r.gdp_real_growth_pct)
  ...
  where r.real_cash_return_pct is not null
    and r.gdp_real_growth_pct is not null
