{# OpenSnow requires a bare, unqualified, unquoted relation name everywhere
   (no catalog/schema, no quotes). Render `relation.identifier` directly — it is
   the raw table name regardless of relation class or include/quote policy. #}
{% macro opensnow__create_table_as(temporary, relation, sql) -%}
  {%- set tmp = "temporary " if temporary else "" -%}
  create {{ tmp }}table {{ relation.identifier }}
  as (
    {{ sql }}
  );
{%- endmacro %}
...
{% macro opensnow__drop_relation(relation) -%}
  {% call statement('drop_relation', auto_begin=False) %}
    {%- if relation.type == 'view' -%}
      drop view if exists {{ relation.identifier }}
    {%- else -%}
